...
unsigned long moveDuration = 0; // Длительность движения в мс
bool isMoving = false;
...
void loop()
{
    ...
    if (lora.available())
    {
        ...
        if (msg.length() > 0)
        {
            ...
            {
                stopRover();
                isMoving = false; // Сбрасываем флаг, отменяя текущий таймер
            }
            // если ровер стоит
            else if (!isMoving) 
            {
                ...
                if (command == 'A') { leftRover();     isMoving = true; roverStartTime = millis(); }
                if (command == 'D') { rightRover();    isMoving = true; roverStartTime = millis(); }
            }
        }
    }

    if (isMoving && (millis() - roverStartTime >= moveDuration))
    {
        stopRover();
        isMoving = false;
    }
